...
const TermsOfUse: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTerms = async () => {
      try {
        const response = await api.get('/lgpd/terms');
          ...
          <div className="backdrop-blur-xl bg-white/90 rounded-3xl shadow-2xl border border-white/40 overflow-hidden">
            ...
            <div className="bg-primary-50 px-8 py-4 border-b border-primary-100">
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  { icon: FileText, label: 'Objeto', href: '#objeto' },
                  { icon: CheckCircle, label: 'Aceitacao', href: '#aceitacao' },
                  { icon: Users, label: 'Responsabilidades', href: '#responsabilidades' },
                  { icon: Ban, label: 'Restricoes', href: '#restricoes' },
                  { icon: AlertCircle, label: 'Limitacoes', href: '#limitacoes' },
                  { icon: Clock, label: 'Vigencia', href: '#vigencia' },
                ].map((item) => (
                  <a
                    key={item.href}
                    href={item.href}
            ...
            <div className="px-8 py-8">
              {terms?.content ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-neutral-600">Termos de uso nao disponiveis no momento.</p>
                </div>
              )}
            </div>
            ...
            <div className="bg-neutral-50 px-8 py-6 border-t border-neutral-200">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <p className="text-sm text-neutral-500">
                  Ao utilizar o AdvWell, voce concorda com estes termos e com nossa Politica de Privacidade.
                </p>
                <Link
