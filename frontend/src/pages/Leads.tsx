              ...
              <div className="desktop-table-view overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-neutral-200 dark:divide-slate-700 bg-white dark:bg-slate-800">
                    {leads.map((lead) => (
                      <tr key={lead.id} className="odd:bg-white even:bg-neutral-50 dark:odd:bg-slate-800 dark:even:bg-slate-700 hover:bg-neutral-100 dark:hover:bg-slate-600 transition-colors">
              ...
              <div className="pagination-controls">
                ...
                <div className="pagination-numbers">
                  {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                    ...
                    return (
                      <button key={pageNum} onClick={() => setPage(pageNum)} className={`pagination-number ${page === pageNum ? 'pagination-number-active' : ''}`}>
                        {pageNum}
                      </button>
                    );
                  })}
                ...
                <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="pagination-btn">
                  <span className="hidden sm:inline">Próximo</span> <ChevronRight className="w-4 h-4" />
                </button>
              </div>
            </div>
          )}
