...
export default function Adverses() {
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await api.get('/adverses/export/csv', {
        params,
        responseType: 'blob',
      });
      ...
      link.setAttribute('download', `adversos_${dateStr}.csv`);
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);

      toast.success('Adversos exportados com sucesso!');
    } catch (error) {
      console.error('Erro ao exportar:', error);
      toast.error('Erro ao exportar adversos');
    } finally {
      setExporting(false);
    }
  };
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const response = await api.post('/adverses/import/csv', formData, {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow dark:shadow-slate-700/20-xl w-full max-w-lg">
            <div className="p-6 border-b border-neutral-200 dark:border-slate-700 flex items-center justify-between">
              <h2 className="text-xl font-semibold text-neutral-800 dark:text-slate-200">Importar Adversos de CSV</h2>
              <button
                onClick={() => {
                  setShowImportModal(false);
                  setImportFile(null);
            ...
            <div className="p-6 space-y-4">
              <div className="bg-info-50 border border-info-200 rounded-lg p-4">
                ...
                <code className="text-xs bg-info-100 px-2 py-1 rounded block overflow-x-auto">
                  Tipo Pessoa;Nome;CPF/CNPJ;RG;Email;Telefone;Cidade;Estado
                </code>
